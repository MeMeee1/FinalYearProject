    ...
    <div className="w-full max-w-[1400px] mx-auto px-2 sm:px-4 lg:px-6 py-4 sm:py-6">
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        <Link href="/dashboard/orders">
          <span className={`px-3 py-2 rounded-md text-sm whitespace-nowrap ${statusFilter === 'all' ? 'bg-blue-600 text-white' : 'bg-white border text-slate-700'
            }`}>All ({stats.total})</span>
        </Link>
        <Link href="/dashboard/orders?status=pending">
          <span className={`px-3 py-2 rounded-md text-sm whitespace-nowrap ${statusFilter === 'pending' ? 'bg-blue-600 text-white' : 'bg-white border text-slate-700'
            }`}>Pending ({stats.pending})</span>
        </Link>
        <Link href="/dashboard/orders?status=processing">
          <span className={`px-3 py-2 rounded-md text-sm whitespace-nowrap ${statusFilter === 'processing' ? 'bg-blue-600 text-white' : 'bg-white border text-slate-700'
            }`}>Processing ({stats.processing})</span>
        </Link>
        <Link href="/dashboard/orders?status=delivered">
          <span className={`px-3 py-2 rounded-md text-sm whitespace-nowrap ${statusFilter === 'delivered' ? 'bg-blue-600 text-white' : 'bg-white border text-slate-700'
                ...
                {filteredOrders.map((order: any) => (
                  <tr key={order.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="p-3 sm:p-4">
                      <span className={`px-2 py-1 rounded text-xs ${order.status === 'delivered' ? 'bg-green-100 text-green-700' :
